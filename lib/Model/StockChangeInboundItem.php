...
/**
 * StockChangeInboundItem Class Doc Comment
 *
 * @category Class
 * @package  kruegge82\jtlffn
 * @author   OpenAPI Generator team
 ...
 */
class StockChangeInboundItem implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'StockChangeInboundItem';
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if ((mb_strlen($this->container['inbound_id']) < 0)) {
            $invalidProperties[] = "invalid value for 'inbound_id', the character length must be bigger than or equal to 0.";
        }

        if (!is_null($this->container['inbound_item_id']) && (mb_strlen($this->container['inbound_item_id']) > 255)) {
            $invalidProperties[] = "invalid value for 'inbound_item_id', the character length must be smaller than or equal to 255.";
        }

        if (!is_null($this->container['inbound_item_id']) && (mb_strlen($this->container['inbound_item_id']) < 0)) {
     ...
     * Sets inbound_id
     *
     * @param string $inbound_id inbound_id
     *
     * @return self
     */
    public function setInboundId($inbound_id)
    {
        ...
        if ((mb_strlen($inbound_id) > 255)) {
            throw new \InvalidArgumentException('invalid length for $inbound_id when calling StockChangeInboundItem., must be smaller than or equal to 255.');
        }
        if ((mb_strlen($inbound_id) < 0)) {
            throw new \InvalidArgumentException('invalid length for $inbound_id when calling StockChangeInboundItem., must be bigger than or equal to 0.');
        }
     ...
     *
     * @param string|null $inbound_item_id inbound_item_id
     *
     * @return self
     */
    public function setInboundItemId($inbound_item_id)
    {
        if (is_null($inbound_item_id)) {
            array_push($this->openAPINullablesSetToNull, 'inbound_item_id');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('inbound_item_id', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
        }
        if (!is_null($inbound_item_id) && (mb_strlen($inbound_item_id) > 255)) {
            throw new \InvalidArgumentException('invalid length for $inbound_item_id when calling StockChangeInboundItem., must be smaller than or equal to 255.');
        }
        if (!is_null($inbound_item_id) && (mb_strlen($inbound_item_id) < 0)) {
            throw new \InvalidArgumentException('invalid length for $inbound_item_id when calling StockChangeInboundItem., must be bigger than or equal to 0.');
        }
